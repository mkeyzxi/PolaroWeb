...
const PhotoPrintsPage: React.FC = () => {
  ...
  // Load daftar kamera setelah izin default
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!allowedLayouts.includes(validType)) validType = "2r";
  ...
  // Ambil foto ke slot
  const takePhoto = () => {
    if (!videoRef.current || !canvasRef.current || photoCount >= maxPhotos)
      return;
    ...
    <div className="p-4 text-center md:mt-15 mb-20 md:mb-0">
      ...
      <div className="flex justify-between gap-5 px-5 md:gap-2 fixed backdrop-blur-sm md:backdrop-blur-none rounded-2xl bottom-0 md:bg-transparent p-3 md:bottom-5 left-0 right-0 md:justify-center flex-wrap mt-4 items-center">
        ...
        <button
          ...
          onClick={() => {
            if (videoRef.current)
              startCamera(selectedDevice || null, videoRef.current);
          }}
        >
        ...
        >
          <Camera />
          <div className="hidden md:block">Ambil Gambar</div>
        </button>
...
export default PhotoPrintsPage;
