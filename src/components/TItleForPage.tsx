...
const TitleForPage: React.FC<TitleForPagePropsType> = ({category, header, subHeader }) => {
	return <div className="flex flex-col mb-5 justify-center items-center md:flex-row md:justify-between px-10 gap-2">
		<h1 className="text-3xl font-bold  text-center mb-2">{header}</h1>
		<div className="flex flex-row justify-center md:justify-end gap-5 md:gap-7">
			{subHeader && subHeader.map((item, index) => {
			return <a href={`/${category}/${item}`} key={index} className="inline-block  text-sm px-2 py-1  w-full text-center  font-bold my-1   text-gray-700 bg-gray-100 border-b-4 hover:bg-gray-200 hover:text-gray-900 transition-colors duration-200">{item}</a>
		})}
		</div>
	</div>;
}
